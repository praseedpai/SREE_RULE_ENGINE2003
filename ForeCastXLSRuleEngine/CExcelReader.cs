...
namespace ForeCastWatcher
{
	...
	public class CExcelReader
	{
		...
		private OracleConnection conn_oracle=null;   // Connection to Oracle 
			...
			int rec1,int rec2 )
		{
			...
			xt = Math.Round(xt,GlobalSettings.num_decimal_places);
			...
			int rec1,int rec2,int ScaleFactor)
		{
			DataTable t = (DataTable)m_Table[SheetName+"$"];  
			double dtr=0;
			int RoundFactor = GlobalSettings.num_decimal_places; 
			for(int j=rec1; j<= rec2;++j)
			{
